...
func TestStackDeploy(t *testing.T) {
	Convey("docker swarm all-in-one test suite =)", t, func() {
		...
		resultStackDeploy := icmd.RunCommand("docker", "-H", "tcp://localhost:7070", "stack", "deploy", "--compose-file", "stack-deploy/docker-compose.yaml", "stack-test")
		...
		go func() {
			command := icmd.Cmd{Command: append([]string{"docker", "-H", "tcp://localhost:7070", "service", "logs", "stack-test_redis", "-f"})}
			resultServiceLogs = icmd.StartCmd(command)
			time.Sleep(2 * time.Second)
			resultServiceLogs.Cmd.Process.Signal(os.Interrupt)
			done <- resultServiceLogs.Combined()
		}()
